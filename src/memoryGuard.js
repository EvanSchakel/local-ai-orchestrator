...
async function getAvailableMemoryGB() {
  try {
    ...
    const freePages = parse(lines, 'Pages free:');
    const inactivePages = parse(lines, 'Pages inactive:');
    const speculativePages = parse(lines, 'Pages speculative:');
 ...
 * @param {number|null} availableMemoryGB - optional pre-fetched available memory to avoid shell call
